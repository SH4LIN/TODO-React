	...
	class Movie_Library_Meta_Boxes {
			...
			<div class = "rt-media-meta-fields rt-media-meta-images">
				<div class = "rt-media-meta-container rt-media-meta-images-container rt-media-meta-uploaded-images-container">
					...
					if ( isset( $rt_media_meta_images_data_attachment_ids ) && ! empty( $rt_media_meta_images_data_attachment_ids[ 0 ] ) ) {
						?>
						<input name = "rt-media-meta-uploaded-images" value = "<?php echo esc_attr(
							json_encode( $rt_media_meta_images_data_attachment_ids[ 0 ] )
						) ?>" hidden = "hidden">
						<h3 class = "rt-media-meta-heading rt-media-meta-images-heading rt-media-meta-uploaded-images-heading"><?php esc_html_e(
								'Uploaded Images'
							) ?></h3>
						<?php
			...
			<div class = "rt-media-meta-fields rt-media-meta-videos">
				<div class = "rt-media-meta-container rt-media-meta-videos-container rt-media-meta-uploaded-videos-container">
					...
					if ( isset( $rt_media_meta_videos_data_attachment_ids ) && ! empty( $rt_media_meta_videos_data_attachment_ids[ 0 ] ) ) {
						?>
						<input name = "rt-media-meta-uploaded-videos" value = "<?php echo esc_attr(
							json_encode( $rt_media_meta_videos_data_attachment_ids[ 0 ] )
						) ?>" hidden = "hidden">
						<h3 class = "rt-media-meta-heading rt-media-meta-videos-heading rt-media-meta-uploaded-videos-heading"><?php esc_html_e(
								'Uploaded Videos'
							) ?></h3>
						<?php
		...
		public function rt_movie_meta_crew( WP_Post $post ): void {
			...
			<div class = "rt-movie-meta-fields rt-movie-meta-crew-fields">
				<?php
				foreach ( $rt_people_data as $key => $rt_person_data ) {
					if ( empty( $rt_person_data ) ) {
						continue;
					}
					?>
					<div class = "rt-movie-meta-container rt-movie-meta-crew-container <?php echo esc_attr(
						strtolower( 'rt-movie-meta-crew-' . $key . '-container' ) ) ?>">
						<label class = "rt-movie-meta-label rt-movie-meta-crew-label <?php echo esc_attr(
							strtolower( 'rt-movie-meta-crew-' . $key . '-label' ) ) ?>" for = "<?php echo esc_attr(
						...
						<select class = "rt-movie-meta-field rt-movie-meta-crew-field <?php echo esc_attr(
							...
							foreach ( $rt_person_data as $rt_p ) {
								if ( empty(
								$selected_people[ strtolower(
									'rt-movie-meta-crew-' . $key
								) ]
								) ) {
									printf(
									...
									$selected = in_array(
										$rt_p[ 'id' ],
										$selected_people[ strtolower( 'rt-movie-meta-crew-' . $key ) ]
									) ? 'selected' : '';
									printf(
						...
						<?php if ( $key === 'Actor' ) { ?>
							<div id = "rt_movie_meta_crew_actor_character_container">
								<?php if ( isset( $rt_movie_meta_crew_data[ 'rt-movie-meta-crew-actor' ] ) ) {
									foreach (
										$rt_movie_meta_crew_data[ 'rt-movie-meta-crew-actor' ] as $rt_character_data
									) {
										?>
		...
		private function un_shift( $array ): array {
